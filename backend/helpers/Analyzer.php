...
class Analyzer {
    ...
    public function analyze($text, $url = '') {
        // --- 1. Basic Sanity/Validation Check ---
        ...
        // If text is too short or contains impossibly long words (gibberish)
        if ($wordCount < 4 || $maxWordLen > 30) {
            return [
                'confidence' => 0,
                'status' => 'invalid',
                'status_ar' => "نص غير صالح للتحليل (أدخل خبراً حقيقياً)",
                'matched_keywords' => [],
                'is_trusted_source' => false,
                'source_name' => '',
                'reasons' => ["النص المدخل يبدو عشوائياً أو قصيراً جداً ليتم اعتباره خبراً."]
            ];
        }
